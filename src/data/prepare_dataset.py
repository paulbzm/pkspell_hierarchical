...
def test_score(score):
    ...
    for event in score.flat:
        #print(event)
        if isinstance(event, m21.bar.Barline):
            print(1)
        else:
            print(2)
    return 0

def endOfMeasureBin(score):
    mesure_numbers = np.array([n.measureNumber for n in score.flat.notes  for p in n.pitches])
    mesure_numbers_shifted = np.append(mesure_numbers,0)[1:]
    endOfMeasure = mesure_numbers != mesure_numbers_shifted
    return endOfMeasure
...
def score2tpc(score):
    ...
    """
    #print([p.name for n in score.flat.notes for p in n.pitches])
    ...
    for event in score.flat:
        ...
        if isinstance(event, m21.bar.Barline):
            ...
            liste.append([p.name for p in event.pitches])
    #print(liste)
    return liste
